...
import {
  ChevronLeft, Send, Scissors, Sparkles, MapPin, Phone, Star, Database,
  Clock, Shield, CheckCircle, Users, UserCircle, ImageIcon, Type, Briefcase,
  AlertCircle, Palette, Link as LinkIcon, CreditCard, CalendarDays, Wand2,
  Rocket, ExternalLink, Save, ShieldCheck, Footprints, PhoneCall, Hammer, Globe,
} from 'lucide-react';
...
// ── Booking style display helpers ──────────────────────────────────────────────
const BOOKING_STYLE_META = {
  digital: { label: 'Digital',     icon: Globe,      color: null          },
  walkin:  { label: 'Walk-in',     icon: Footprints, color: null          },
  hybrid:  { label: 'Call to Book',icon: PhoneCall,  color: '#63b3ed'     },
  build:   { label: 'CBA Setup',   icon: Hammer,     color: '#a78bfa'     },
};
...
function PublishConfirmModal({ bizId, onConfirm, onCancel, loading }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-5" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)' }}>
      <div
        className="w-full max-w-sm rounded-2xl border overflow-hidden animate-fade-up shadow-2xl"
        style={{ borderColor: 'rgba(201,162,39,0.4)', background: 'var(--bg-card)' }}
      >
        {/* Header */}
        <div className="px-5 py-4 border-b flex items-center gap-3" style={{ borderColor: 'rgba(201,162,39,0.2)', background: 'rgba(201,162,39,0.06)' }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(201,162,39,0.15)' }}>
            <Rocket className="w-4 h-4" style={{ color: 'var(--gold)' }} />
          </div>
          <div>
            <p className="text-xs font-bold" style={{ color: 'var(--gold)' }}>Confirm Live Publish</p>
            <p className="text-[10px]" style={{ color: 'var(--text-faint)' }}>Master Mode — Admin Action</p>
          </div>
        </div>
        ...
        <div className="px-5 py-5">
          <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-secondary)' }}>
            Are you sure? This will update the live website at:
          </p>
          <div className="rounded-xl border px-3.5 py-2.5 mb-5 flex items-center gap-2" style={{ borderColor: 'rgba(201,162,39,0.25)', background: 'rgba(201,162,39,0.05)' }}>
            <ExternalLink className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--gold)' }} />
            <span className="text-xs font-mono font-semibold truncate" style={{ color: 'var(--gold)' }}>{liveUrl}</span>
          </div>
          <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-faint)' }}>
            This action is <strong style={{ color: 'var(--text-secondary)' }}>instant</strong> — the live site will reflect all current data immediately upon confirmation.
          </p>
        </div>
        ...
        <div className="px-5 pb-5 flex gap-2.5">
          <button
            onClick={onCancel}
            disabled={loading}
            className="flex-1 py-3 rounded-full border text-sm font-medium transition-all active:scale-95"
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-full text-sm font-bold text-white uppercase tracking-wider transition-all active:scale-95 disabled:opacity-60"
            style={{ background: 'var(--gold)', boxShadow: '0 4px 16px rgba(201,162,39,0.35)' }}
          >
            {loading ? (
...
function AdminPublishSuccess({ data, onBack, clientId }) {
  ...
  useEffect(() => {
    const id = setInterval(() => setTick((t) => (t < 100 ? t + 2 : t)), 25);
    return () => clearInterval(id);
  }, []);

  return (
    <div className="min-h-svh flex flex-col items-center justify-center px-5 py-14 text-center relative overflow-hidden" style={{ background: '#0a1a10' }}>
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(ellipse, rgba(201,162,39,0.12) 0%, transparent 70%)', filter: 'blur(40px)' }} />
      ...
      <div className="relative z-10 animate-fade-up mb-8">
        <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full border" style={{ borderColor: 'rgba(201,162,39,0.3)', background: 'rgba(201,162,39,0.08)' }}>
          <ShieldCheck className="w-3.5 h-3.5" style={{ color: 'var(--gold)' }} />
          <span className="text-[11px] tracking-[0.25em] uppercase font-bold" style={{ color: 'var(--gold)' }}>Master Mode · Live</span>
        </div>
      </div>
      ...
      <p className="relative z-10 animate-fade-up delay-200 text-sm max-w-[300px] leading-relaxed mb-8" style={{ color: 'var(--text-secondary)' }}>
        All brand data has been written to the database and the live site has been updated for{' '}
        <span style={{ color: 'var(--text-primary)' }}>{data.businessName || 'this business'}</span>.
      </p>

      {/* Deploy meter */}
      <div className="relative z-10 animate-fade-up delay-300 w-full max-w-sm rounded-2xl border overflow-hidden mb-6" style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)' }}>
        <div className="px-5 py-3.5 border-b flex items-center justify-between" style={{ borderColor: 'var(--border-sub)', background: 'var(--bg-raised)' }}>
          <div className="flex items-center gap-2">
            <Rocket className="w-3.5 h-3.5" style={{ color: 'var(--gold)' }} />
            <span className="text-[10px] tracking-widest uppercase font-bold" style={{ color: 'var(--gold)' }}>Deploy Status</span>
          </div>
          <span className="text-[11px] font-mono" style={{ color: tick >= 100 ? '#4ade80' : 'var(--text-muted)' }}>
            {tick >= 100 ? 'LIVE' : `${tick}%`}
          </span>
        </div>
        ...
        <div className="px-5 py-4 flex flex-col gap-2" style={{ background: 'var(--bg-card)' }}>
          {['Brief saved to database','Staff records synced','Services published','is_published: true'].map((item, i) => (
            <div key={i} className="flex items-center justify-between animate-fade-up" style={{ animationDelay: `${0.4 + i * 0.08}s` }}>
      ...
      {/* Live link CTA */}
      <div className="relative z-10 animate-fade-up delay-500 w-full max-w-sm mb-8">
        <a
          href={liveUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2.5 w-full py-4 rounded-full font-bold text-sm uppercase tracking-wider transition-all hover:scale-[1.015] active:scale-95"
          style={{ background: 'var(--gold)', color: '#0a1208', boxShadow: '0 4px 24px rgba(201,162,39,0.4)' }}
        >
          <ExternalLink className="w-4 h-4" />
          View Live Site
        </a>
        <p className="text-[10px] mt-2 font-mono" style={{ color: 'var(--text-faint)' }}>{liveUrl}</p>
      </div>

      <button
        onClick={onBack}
        className="relative z-10 animate-fade-up delay-600 flex items-center gap-2 px-5 py-3 rounded-full border text-sm font-medium transition-all active:scale-95"
        style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}
      >
...
function CBALabPage({ data, clientId }) {
  ...
  return (
    <div className="min-h-svh flex flex-col items-center justify-center px-5 py-14 text-center relative overflow-hidden" style={{ background: '#0e1a26' }}>
      ...
      <div className="relative z-10 animate-fade-up mb-10">
        <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full border" style={{ borderColor: 'rgba(201,162,39,0.25)', background: 'rgba(201,162,39,0.07)' }}>
          <div className="flex gap-1">
            {[0, 0.2, 0.4].map((d, i) => <div key={i} className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: 'var(--gold)', animationDelay: `${d}s` }} />)}
          </div>
      ...
      <div className="relative z-10 animate-fade-up delay-100 mb-4 max-w-sm">
        <p className="text-[11px] tracking-[0.2em] uppercase font-semibold mb-3" style={{ color: 'var(--coral)' }}>Brief Saved</p>
        <h1 className="font-serif-display text-3xl sm:text-4xl font-bold leading-tight tracking-tight" style={{ color: 'var(--text-primary)' }}>
      ...
      <p className="relative z-10 animate-fade-up delay-200 text-sm max-w-[300px] leading-relaxed mb-4 italic" style={{ color: 'var(--text-secondary)' }}>
      ...
      <div className="relative z-10 animate-fade-up delay-250 w-full max-w-sm mb-8 rounded-xl border px-4 py-3.5 flex items-start gap-3 text-left" style={{ borderColor: 'rgba(201,162,39,0.2)', background: 'rgba(201,162,39,0.05)' }}>
        <Save className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--gold)' }} />
        <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
          Your progress has been saved. The <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>Brewing a Brand</span> team will review your details before going live.
        </p>
      </div>
            ...
            <div key={i} className="flex items-center justify-between animate-fade-up" style={{ animationDelay: `${0.4 + i * 0.1}s` }}>
              ...
              {item.done ? (
                ...
                <div className="flex items-center gap-1">
                  {[0,0.15,0.3].map((d,j) => <div key={j} className="w-1 h-1 rounded-full animate-pulse" style={{ background: 'var(--border)', animationDelay: `${d}s` }} />)}
                </div>
              )}
            </div>
          ))}
...
// ── Main component ─────────────────────────────────────────────────────────────
export default function Step5Finalize({ onBack, data, isAdmin }) {
  const [submitted,      setSubmitted]      = useState(false);   // client save done
  const [published,      setPublished]      = useState(false);   // admin publish done
      ...
      <div className="px-5 py-8 max-w-lg mx-auto w-full">
        ...
        <div className="animate-fade-up mb-6">
          ...
          <h2 className="font-serif-display text-2xl font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>
            Review {isAdmin ? '& Deploy' : 'Your Brief'}
          </h2>
          ...
          <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
            {isAdmin
              ? 'Review the complete brief. Use Republish to push all changes live instantly, or save as pending for client review.'
        ...
        <div className="animate-fade-up delay-100 rounded-2xl border overflow-hidden mb-5" style={{ borderColor: 'var(--border)', background: 'var(--bg-card)' }}>
          ...
          {data.brandColors && (
            <div className="px-5 py-3.5 border-b" style={{ borderColor: 'var(--border-sub)', background: 'var(--bg-surface)' }}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {[{ label: 'Primary', val: data.brandColors.primary }, { label: 'Secondary', val: data.brandColors.secondary }, { label: 'Accent', val: data.brandColors.accent }].map(({ label, val }) => val && (
            ...
            <div className="px-5 py-3.5 border-b" style={{ borderColor: 'var(--border-sub)', background: 'var(--bg-surface)' }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {data.paymentMethods.map((p) => (
                  <span key={p} className="px-2.5 py-1 rounded-full text-[10px] font-semibold border" style={{ borderColor: `${accent}35`, background: `${accent}0d`, color: accent }}>
          ...
          {hasDiscovery && (
            <div className="px-5 py-3.5 border-b" style={{ borderColor: 'var(--border-sub)', background: 'var(--bg-surface)' }}>
              ...
              <div className="flex flex-col gap-1.5">
                {data.address && <div className="flex items-center gap-2"><MapPin className="w-3 h-3 flex-shrink-0" style={{ color: 'var(--text-muted)' }} /><span className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>{data.address}</span></div>}
                {data.phone   && <div className="flex items-center gap-2"><Phone className="w-3 h-3 flex-shrink-0" style={{ color: 'var(--text-muted)' }} /><span className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>{data.phone}</span></div>}
                {data.rating != null && (
                  <div className="flex items-center gap-2">
          ...
          <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border-sub)' }}>
            <p className="text-[10px] uppercase tracking-widest mb-3 font-semibold" style={{ color: 'var(--text-muted)' }}>
              Service Menu ({selectedServices.length})
            </p>
            ...
            {(data.staff || []).length > 0 ? (
              <div className="flex flex-col gap-2">
                {data.staff.map((member) => {
                  const styleMeta = BOOKING_STYLE_META[member.bookingStyle || 'digital'];
                  ...
                  return (
                    <div key={member.id} className="rounded-xl overflow-hidden border" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-sub)' }}>
                      <div className="flex items-center gap-3 px-3 py-2.5">
                        {/* Photo — only shown if portfolioAccess */}
                        {member.portfolioAccess ? (
                          ...
                          <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 border" style={{ borderColor: 'var(--border)', background: 'var(--bg-raised)' }}>
                            <UserCircle className="w-3.5 h-3.5" style={{ color: 'var(--text-faint)' }} />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {member.portfolioAccess && (
                              <span className="text-[8px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded border flex-shrink-0"
                                style={{ borderColor: 'rgba(201,162,39,0.3)', background: 'rgba(201,162,39,0.08)', color: 'var(--gold)' }}>
                                Portfolio
                              </span>
                            )}
                          </div>
                        ...
                        {/* Booking style chip */}
                        <div className="flex items-center gap-1 flex-shrink-0 px-2 py-1 rounded-full border"
                          style={{
                            borderColor: styleMeta.color ? `${styleMeta.color}30` : `${accent}25`,
                            background:  styleMeta.color ? `${styleMeta.color}08` : `${accent}08`,
                            color:       styleMeta.color || accent,
                          }}>
                          <StyleIcon className="w-2.5 h-2.5" />
                          <span className="text-[9px] font-semibold">{styleMeta.label}</span>
                        </div>
                      </div>
                      {/* Bio */}
                      {member.bio && (
                      ...
                      {/* Instagram — only if portfolioAccess */}
                      {member.portfolioAccess && member.instagram && (
                        <div className="px-3 pb-1.5 flex items-center gap-1.5">
                          <span className="text-[10px]" style={{ color: '#E1306C' }}>@{member.instagram}</span>
                        </div>
                      )}
                      {/* Digital booking link */}
                      {member.bookingStyle === 'digital' && member.bookingLink && (
                ...
                {data.hiring.roles.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {data.hiring.roles.map((role) => (
                      <span key={role} className="px-2.5 py-1 rounded-full text-[10px] font-medium border" style={{ borderColor: `${accent}40`, background: `${accent}0d`, color: `${accent}cc` }}>{role}</span>
                    ))}
                  </div>
                )}
        ...
        <div className="animate-fade-up delay-300 mb-5">
          ...
          {isAdmin ? (
            ...
            <div className="flex flex-col gap-3">
              {/* Gatekeeper note */}
              <div className="rounded-xl border px-4 py-3.5 flex items-start gap-3" style={{ borderColor: 'rgba(201,162,39,0.25)', background: 'rgba(201,162,39,0.04)' }}>
                <Shield className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--gold)' }} />
                <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                  The live site <strong style={{ color: 'var(--text-primary)' }}>will not update</strong> until you explicitly hit Republish. Saving progress keeps the record in a <span style={{ color: 'var(--gold)' }}>pending</span> state.
                </p>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-4 rounded-full font-bold text-sm uppercase tracking-wider transition-all duration-200 hover:scale-[1.015] active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
                style={{ background: 'var(--gold)', color: '#0a0c08', boxShadow: '0 4px 24px rgba(201,162,39,0.4)' }}
              >
                <Rocket className="w-4 h-4" />
                Republish Live Site
              </button>

              {/* Secondary: Save as Pending */}
              <button
                onClick={handleSave}
                disabled={loading}
                className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-full border font-semibold text-sm transition-all duration-200 active:scale-95 disabled:opacity-60"
                style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}
              >
                {loading ? (
                  <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none">
            ...
            <div className="flex flex-col gap-4">
              ...
              <button
                onClick={handleSave}
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-4 rounded-full font-bold text-sm text-white uppercase tracking-wider transition-all duration-200 hover:scale-[1.015] active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
          ...
          {submitError && !loading && (
            <div className="mt-4 flex items-start gap-2.5 rounded-xl border px-4 py-3 animate-fade-up" style={{ borderColor: 'rgba(248,113,113,0.35)', background: 'rgba(248,113,113,0.07)' }}>
              ...
              <div>
                <p className="text-xs font-semibold text-red-400 mb-0.5">Action Failed</p>
